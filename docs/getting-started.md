...
Console needs **Go 1.22+** to build. There is no cgo dependency — the embedded
database is the pure-Go [`modernc.org/sqlite`](https://pkg.go.dev/modernc.org/sqlite),
so the result is a truly static binary.
...
| `CONSOLE_LLM_PROVIDER` | `anthropic` | LLM provider for AI-Assisted onboarding (`""` to disable) |
...
the same result. Try a few subject IDs and you will see a stable ~50% split.
...
```bash
./console serve                 # listens on :8080 by default
...
- [Plugins](plugins.md) — add a storage / status / LLM backend.
